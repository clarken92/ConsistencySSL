...
from models.semi_supervised.mean_teacher import MeanTeacher_MUR
...
from my_utils.python_utils.annealing import StepAnnealing, SigmoidRampup, SigmoidRampdown
...
# This is the default settings for CIFAR10
...
parser.add_argument('--input_norm', type=str, default="applied",
...
# For Nesterov momentum optimizer
...
parser.add_argument('--cons_mode', type=str, default="mse", choices=["mse", "kld", "rev_kld", "2rand"])
parser.add_argument('--cons_4_unlabeled_only', type=str2bool, default=False)
parser.add_argument('--update_ops_4_stu_only', type=str2bool, default=True)
parser.add_argument('--mask_weights', type=str2bool, default=False)
parser.add_argument('--ema_4_log_sigma2', type=str2bool, default=False)
parser.add_argument('--cons_against_mean', type=str2bool, default=True)
...
parser.add_argument('--ema_momentum_init', type=float, default=0.99)
parser.add_argument('--ema_momentum_final', type=float, default=0.99)

parser.add_argument('--mur_coeff_max', type=float, default=4.0)
parser.add_argument('--mur_4_unlabeled_only', type=str2bool, default=False)

parser.add_argument('--mur_iter_mode', type=str, default="proj_grad_asc",
                    choices=["grad_asc_w_lagrangian_relax", "proj_grad_asc"])
parser.add_argument('--mur_opt_steps', type=int, default=1)
parser.add_argument('--mur_opt_lr', type=float, default=0.01)
parser.add_argument('--mur_noise_radius', type=float, default=10.0)
...
def main(args):
    ...
    # Hyperparameters
    # ===================================== #
    hyper_updater = HyperParamUpdater(['lr', 'ema_momentum',
                                       'cent_u_coeff', 'cons_coeff', 'mur_coeff'],
                                      [args.lr_max, args.ema_momentum_init,
                                       args.cent_u_coeff_max, args.cons_coeff_max,
                                       args.mur_coeff_max],
    ...
    if args.model_name == "9310gaurav":
        if args.weight_norm:
            raise ValueError("Since weight norm causes erratic performance, "
                             "we do not use weight norm!")
        else:
    ...
    # Main model
    # ------------------------------------- #
    model = MeanTeacher_MUR(x_shape=x_shape, y_shape=num_classes,
                            ...
                            mur_4_unlabeled_only=args.mur_4_unlabeled_only,
                            mur_noise_radius=args.mur_noise_radius,
                            mur_opt_steps=args.mur_opt_steps,
                            mur_iter_mode=args.mur_iter_mode,
                            mur_opt_lr=args.mur_opt_lr)

    loss_coeff_dict = {
        'cross_ent_l': args.cross_ent_l,
        'cond_ent_u': hyper_updater.variables['cent_u_coeff'],
        'cons': hyper_updater.variables['cons_coeff'],
        'mur': hyper_updater.variables['mur_coeff'],
    }
    ...
    fetch_keys_AE_u = ['acc_y_u', 'cond_ent_u', 'cons', 'mur', 'l2_reg', 'grad_norm_avg']
    ...
    # Annealer
    # ------------------------------------- #
    step_rampup_annealer = StepAnnealing(args.rampup_len_step, value_0=0, value_1=1)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in range(batches_per_epoch):
            ...
            ema_momentum = (1.0 - step_rampup) * args.ema_momentum_init + step_rampup * args.ema_momentum_final
            cent_u_coeff = sigmoid_rampup * args.cent_u_coeff_max
            cons_coeff = sigmoid_rampup * args.cons_coeff_max
            mur_coeff = sigmoid_rampup * args.mur_coeff_max
            ...
            if global_step % args.log_freq == 0:
                ...
                iter_time_gap = log_time_gap * 1.0 / args.log_freq
