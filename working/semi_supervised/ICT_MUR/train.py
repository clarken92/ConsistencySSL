...
from models.semi_supervised.ict import ICT_MUR
...
parser.add_argument('--mur_noise_radius', type=float, default=6.0)
parser.add_argument('--mur_opt_steps', type=int, default=1)
parser.add_argument('--mur_opt_lr', type=float, default=0.001)
...
def main(args):
    ...
    if args.model_name == "9310gaurav":
        if args.weight_norm:
            # main_classifier = MainClassifier4WNPiModel_9310gaurav(
            #     num_classes=num_classes, use_gauss_noise=args.gauss_noise)
            raise ValueError("Since weight norm causes erratic "
                             "performance of MeanTeacher so we do not use weight norm!")
        else:
    ...
    # Main model
    # ------------------------------------- #
    model = ICT_MUR(x_shape=x_shape, y_shape=num_classes,
                    ...
                    weight_decay=args.weight_decay, alpha=args.alpha,
    ...
    fetch_keys_AE_u = ['acc_y_u', 'cond_ent_u', 'cons', 'mur', 'l2_reg']
